...
From the `variants_tbl` Create a table that contains all the columns
with the letter "`i`" and column "`POS`", without columns "**Indiv**"
and "**FILTER**".

Hint: look at the help function `conatins()` we've just covered.
...
You can separate them using a `,` as arguments to the function, they
will be combined using the `&` (AND) logical operator.
...
the positions (`POS`) `1e6` (one million) and `2e6`(included)

have `QUAL` greater than 200.
...
the next, which is useful when you need to many things to the same data
set.

It was possible to do this before pipes were added to R, but it was much
messier and more difficult.

Pipes in R look like `|>` ; Previous only available via the magrittr
package, which is installed as part of dply.
...
Retain only the columns `REF`, `ALT`, and `POS`.
...
`sample_id`, `POS`, `QUAL`, and `POLPROB` columns for now. Add a line to
the below code to only show those columns and show the output via
`glimpse()`.
...
difference between the our sequences and the reference genome. The
...
```{r group_by}
# So to view the highest (max()) filtered depth (DP) for each sample (sample_id):
variants %>%
  group_by(sample_id) %>%
  summarize(max(DP))
```
