...
![](images/ggplot2_simplified.webp)
...
To add a main title "Read Depth vs. Position" to the plot, we use labs()
parameter title

```{r}
# add main title using labs parameter title "Read Depth vs. Position
ggplot(data = variants, aes(x = POS, y = DP, color = sample_id)) +
  geom_point(alpha = 0.5) +
  labs(x = "Base Pair Position",
       y = "Read Depth (DP)",
...
Use what you just learned to create a ggplot scatter plot (geom_point)
of y PHRED scaled quality (`QUAL`) over x position (`POS`) with the
`sample_id` showing in different colors.
...
```{r}
# create bar chart for a value using geom_col
variants %>% 
  group_by(sample_id) %>%
