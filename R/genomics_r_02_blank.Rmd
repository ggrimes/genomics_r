...
variable that we measure or report on.
...
```{r}
# read in file combined_tidy_vcf.csv using read.csv()
```
...
that contain text or discrete values), **`summary()`** will provide a
frequency count of the levels/categories present.
...
#se skim() fucntion of variants dataframe
```
...
document which variable could be dfeined as factors.
...
2.  **Data Integrity**: Factors help maintain data integrity by ensuring
    that the data can only take on a limited number of levels. This is
    helpful when you know that a variable should only have certain
    values.
...
5.  **Data Analysis**: Many R functions are designed to work with
    ...
    For example, the **`table()`** function will produce a frequency
    count of the factor levels, and the **`summary()`** function will do
    the same.

So, even though factors in R might seem strange at first, especially to
those coming from other programming languages, they are a powerful tool
for working with categorical data in R.
...
`factor` vector); we will create a new object to avoid confusion using
the `factor()` function:
...
# use str function on column REF
...
# order snps using fct_infreq and fct_rev
```
